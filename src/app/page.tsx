...
const PROJECTS_BASE = [
  ...
  // ... le reste de tes projets reste inchangé
...
// --- COMPOSANT DES LIGNES & POINTS NETS EN PARALLAXE RAPIDE ---
const BackgroundLines = () => {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] });
  
  // Plans de parallaxe des lignes
  const yFast = useTransform(scrollYProgress, [0, 1], [-200, 200]);
  const ySlow = useTransform(scrollYProgress, [0, 1], [100, -100]);
  
  // Vitesse ultra-rapide pour les points blancs (contours nets)
  const ySuperFast = useTransform(scrollYProgress, [0, 1], [-700, 700]);

  // 1. Matrice de lignes (Épaisseur 2px, opacité très basse)
  const lines = useRef(
  ...
  // 2. Sélection restreinte de points (Quantité modérée, tailles variées de 1 à 4px)
  const dots = useRef(
    Array.from({ length: 35 }).map((_, i) => {
      const top = 8 + (i * 2.5);
      const left = (i * 19.3) % 100; // Largeur bien distribuée
      const opacity = 0.15 + ((i * 7) % 25) / 100; // Visibilité subtile mais nette
      
      // Tailles dynamiques de 1px, 2px, 3px ou 4px maximum
      const sizeIndex = i % 4;
      ...
      <div className="relative w-full h-full">
        ...
        {dots.current.map((dot, idx) => (
          <motion.div
            ...
            /* w-[x] h-[x] injecté dynamiquement, bg-white pur sans shadow ni blur */
...
const ParallaxProject = ({ project, index }: { project: any, index: number }) => {
  ...
  return (
    ...
    >
      {/* ZONE IMAGE (Le survol ne fait plus que zoomer/assombrir proprement) */}
      <div className="relative aspect-[4/5] md:aspect-[16/11] bg-neutral-900 border border-white/5 overflow-hidden">
        ...
        />
        {/* Voile sombre au survol pour détacher l'image du fond */}
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
      </div>

      {/* BLOC TEXTE DESSOUS (Épuré et restructuré) */}
      <div className="mt-6 flex justify-between items-start px-1">
          <div className="flex flex-col space-y-1">
              {/* Le thème / catégorie juste au-dessus, discret */}
...
className="text-[12px] tracking-[0.4em] uppercase font-bold text-white/40 hover:text-white transition-colors duration-300 
      ...
      WATCH DEMOREEL
        ...
        {/* CINÉMA / MODAL PLEIN ÉCRAN POUR LA VIDÉO COMPLÈTE */}
{isDemoreelOpen && (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  ...
  >
    ...
    <button 
      ...
      className="absolute top-6 right-6 text-white/50 hover:text-white text-[10px] tracking-[0.3em] uppercase z-51 transition-colors duration-300"
    >
      [ X ]
    </button>

    {/* Conteneur de la vidéo (format 16:9 cinématographique) */}
    <div className="w-full max-w-5xl aspect-video bg-neutral-950 shadow-2xl relative">
      <video 
        src="Demoreel_2026_V1_low2.mp4" // Utilise le même fichier (ou un fichier plus long/lourd si tu veux)
        autoPlay 
        controls // Ajoute les barres de lecture (play, pause, volume) pour le confort
        className="w-full h-full object-contain"
      />
    </div>

    {/* Fond cliquable pour fermer également si on clique à côté de la vidéo */}
    <div 
...
<section id="projects-section" className="relative bg-[#050505] py-40 z-30">
  ...
  {/* Zone parente de la galerie sans fond opaque pour laisser passer les lignes */}
  <div className="relative w-full match-parent-height">
    
    {/* Les lignes sont en z-[1] */}
    <BackgroundLines />

    {/* La grille est forcée en z-10 pour passer au-dessus des lignes */}
    <div className="px-10 md:px-32 grid grid-cols-1 md:grid-cols-12 gap-y-20 md:gap-y-64 max-w-[1500px] mx-auto relative z-10 bg-transparent">
      {PROJECTS_BASE.map((p, i) => (
        <ParallaxProject key={p.id} project={p} index={i} />
      ))}
    </div>
